...
@Slf4j
@Service("beneficiaireTPOfflineBackendDao")
public class BeneficiaireTPOfflineBackendDaoImpl extends BeneficiaireBackendDaoImpl {

  public BeneficiaireTPOfflineBackendDaoImpl(
  ...
  public List<BenefAIV5> findBenefFromRequest(final UniqueAccessPointRequestV5 requete) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "PAU V5, recherche du bénéficiaire pour TP_OFFLINE. Execution de l'aggregation => {}",
  ...
  protected Criteria createCriteriaBenef(
    ...
    final Criteria criteriaNir = new Criteria();
    if (StringUtils.isNotBlank(nirCode)) {
      ...
      criteriaNir.orOperator(nirBeneficiaireCriteria, nirOdCriteria);
    }
    ...
    if (insurer != null) {
      ...
      criteria.andOperator(criteriaAmc, criteriaNir);
    } else {
      criteria.andOperator(criteriaNir);
    }
