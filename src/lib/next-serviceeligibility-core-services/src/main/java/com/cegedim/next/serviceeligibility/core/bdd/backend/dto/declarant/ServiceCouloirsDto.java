package com.cegedim.next.serviceeligibility.core.bdd.backend.dto.declarant;

import com.cegedim.next.serviceeligibility.core.bdd.backend.dto.generic.GenericDto;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import java.util.List;
import lombok.Data;

/** Classe DTO de l'entite {@code ServiceDroits}. */
@JsonInclude(Include.NON_NULL)
@Data
public class ServiceCouloirsDto implements GenericDto, Comparable<ServiceCouloirsDto> {

  private static final long serialVersionUID = 1L;

  /* PROPRIETES */
  private String service;
  private List<String> listCouloirs;

  @Override
  public int compareTo(ServiceCouloirsDto o) {
    return this.service.compareTo(o.getService());
  }
}
