...
public class Declarant extends DocumentEntity implements GenericDomain<Declarant>, Audit {
  ...
  @Getter @Setter private List<Pilotage> pilotages;

  @Getter @Setter private List<String> numerosAMCEchanges;

  @Getter @Setter private List<TranscoDomainesTP> transcodageDomainesTP;

  @Getter @Setter private List<ConventionTP> conventionTP;
  @Getter @Setter private List<CodeRenvoiTP> codeRenvoiTP;
  @Getter @Setter private List<RegroupementDomainesTP> regroupementDomainesTP;
  @Getter @Setter private List<FondCarteTP> fondCarteTP;
  @Getter @Setter private String delaiRetention;

  public Declarant() {
    /* empty constructor */ }

  public Declarant(Declarant source) {
    ...
    if (!CollectionUtils.isEmpty(source.getPilotages())) {
      this.pilotages = new ArrayList<>();
      for (Pilotage pilotage : source.getPilotages()) {
        this.pilotages.add(new Pilotage(pilotage));
      }
    }
    if (!CollectionUtils.isEmpty(source.getNumerosAMCEchanges())) {
      this.numerosAMCEchanges = new ArrayList<>(source.getNumerosAMCEchanges());
    }
  }
  ...
  @Override
  public int compareTo(Declarant declarant) {
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof Declarant other)) {
      return false;
    }
