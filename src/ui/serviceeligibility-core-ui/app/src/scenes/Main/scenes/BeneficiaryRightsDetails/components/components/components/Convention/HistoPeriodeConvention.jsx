...
class HistoriquePeriode extends Component {
    getColumns() {
        ...
        return [
            {
                Header: StringUtils.splitIntoDivs(t('historiqueConvention.periode'), false),
                accessor: 'periode',
                id: 'periode',
                Cell: ({ value }) => (
                    <div>
            ...
            {
                Header: StringUtils.splitIntoDivs(t('historiqueConvention.conventions'), false),
                accessor: 'conventions',
                id: 'conventions',
                Cell: ({ value }) => (
                    <div>
                        {value.map((convention) => (
                            <div>
                                <span className="cgd-comment">{t('beneficiaryRightsDetails.priorite')} :</span>{' '}
                                {`${convention.priorite} : ${utils.getLibelleConverter(
                                    convention.typeConventionnement.code,
                                    convention.typeConventionnement.libelle,
                                )}`}
                            </div>
                        ))}
                    </div>
    ...
    render() {
        ...
        const data = historique.map((item) => ({
            ...
            conventions: item.typeConventionnements,
        }));
        return (
            <Row>
