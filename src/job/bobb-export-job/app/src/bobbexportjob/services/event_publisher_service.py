...
from bobbexportjob.common.common_constants import FILE_NAME, ERROR_MESSAGE
from bobbexportjob.models.event_type import EventType as EventTypes
...
class EventPublisherService:
    ...
    def publish_success_event(self, filename):
        self.handle_publish_event(EventTypes.BOBB_EXPORT_FILE_SUCCEEDED_EVENT, filename, success=True)

    def publish_failed_event(self, error_message):
        self.handle_publish_event(EventTypes.BOBB_EXPORT_FILE_FAILED_EVENT, error_message, success=False)
    ...
    def handle_publish_event(self, event_type, data, success):
        ...
        if success:
            ...
            logger.info(f"Published event: {event_type.value} with information: errorMessage: {event_data[ERROR_MESSAGE]}")

    @staticmethod
    def initialize_event_data(data, success):
        if success:
            event_data = {FILE_NAME: data}
        else:
            event_data = {ERROR_MESSAGE: data}
        return event_data
