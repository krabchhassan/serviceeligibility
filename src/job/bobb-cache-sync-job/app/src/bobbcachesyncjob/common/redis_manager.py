...
from bobbcachesyncjob.common.common_constants import REDIS_AGENT_NODE_0, REDIS_AGENT_NODE_1, REDIS_AGENT_NODE_2, \
    REDIS_ADMIN_PASSWORD, REDIS_ADMIN_USER, REDIS_CRT_PATH, REDIS_PORT, REDIS_MASTER, REDIS_PASSWORD, REDIS_USER
from bobbcachesyncjob.configuration.redis_configuration import RedisConfiguration
...
class RedisManager:
    ...
    def store_single_entry(self, key: str, value: str) -> None:
        """
        Stores a single key-value pair in the Redis database.

        :param key: Identifier of entry.
        :param value: Value of entry.
        :return: No specific return value.
        :raise: Exception handling is not fully implemented. Specific exceptions need to be documented.
        """
        self.master_connection.set(name=key, value=value)

    def store_multiple_entries(self, mapping: Mapping[str, str]) -> None:
        """
        Stores multiple key-value pairs in the Redis database.

        :param mapping: A dictionary mapping keys to their corresponding values.
        :return: None
        :raise: Proper exception handling is pending. Specific exceptions should be documented.
        """
        self.master_connection.mset(mapping)
    ...
    def delete_all_entries(self, pattern: str):
        ...
        while cursor != 0:
            ...
            if keys:
                nb_deleted_values += self.master_connection.delete(*keys)
