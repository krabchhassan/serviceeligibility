...
logger.info("Début du traitement creation de l'index tracesExtractionConso")
logger.info(f"Client Type : {clientType}")
if clientType == 'INSURER':
    try:
        tracesExtractionConso_collection = db["tracesExtractionConso"]
        keys=[('nomFichier', ASCENDING), ]
        tracesExtractionConso_collection.create_index( keys=keys, name="tracesExtractionConso04")
    except Exception as e:
        logger.error(f"probleme creation index : {e}")


logger.info("Fin du traitement creation de l'index tracesExtractionConso")
