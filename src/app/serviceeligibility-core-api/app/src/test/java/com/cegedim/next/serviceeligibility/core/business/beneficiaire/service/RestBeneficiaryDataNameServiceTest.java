...
public class RestBeneficiaryDataNameServiceTest {

  public static final String NUMERO_PERSONNE = "1234567890-321321032";

  public static final String ID_DECLARANT = "1234567890";
  public static final String DEBUT = "2020-01-01";
  public static final String NOM_FAMILLE = "TEST";
  public static final String PRENOM = "UNITAIRE";
  public static final String NOM_USAGE = "JUNIT";

  @Test
  void mapBeneficiaryDataNameFromBenefTest() {
    RequestBeneficiaryDataNameDto request =
        new RequestBeneficiaryDataNameDto(NUMERO_PERSONNE, "32103206664");
    BenefAIV5 benef = getBenef();

    RestBeneficaryDataNameService service = new RestBeneficaryDataNameService();

    BeneficiaryDataNameDto beneficiaryDataNameDto = service.getBeneficiaryDataName(request, benef);
    Assertions.assertEquals(PRENOM, beneficiaryDataNameDto.getFirstName());
    Assertions.assertEquals(NOM_FAMILLE, beneficiaryDataNameDto.getLastName());
    Assertions.assertEquals(NOM_USAGE, beneficiaryDataNameDto.getCommonName());
  }

  private BenefAIV5 getBenef() {
    BenefAIV5 benef = new BenefAIV5();
    Periode periode = new Periode(DEBUT, "2021-12-31");
    ...
    contrats.add(getContratV5("32103206664", NOM_FAMILLE));
    contrats.add(getContratV5("32103206665", NOM_FAMILLE));
    benef.setContrats(contrats);
    List<String> services = new ArrayList<>();
    services.add("ServicePrestation");
    benef.setServices(services);
    benef.setKey(benef.getIdentite().getNumeroPersonne());

    return benef;
  }

  private ContratV5 getContratV5(String numeroContrat, String nom) {
    ContratV5 contrat = new ContratV5();
    contrat.setCodeEtat("V");
    contrat.setNumeroContrat(numeroContrat);
    DataAssure dataAssure = getDataAssure(nom);
    contrat.setData(dataAssure);
    return contrat;
  }

  private DataAssure getDataAssure(String nomFamille) {
    DataAssure dataAssure = new DataAssure();
    NomAssure nom = new NomAssure();
    nom.setCivilite("MR");
    nom.setNomFamille(nomFamille);
    nom.setNomUsage(NOM_USAGE);
    ...
    NomDestinataire nomDest = getNomDest(NOM_FAMILLE);
  ...
  private static NomDestinataire getNomDest(String nom) {
    ...
    nomDest.setNomUsage(NOM_USAGE);
    nomDest.setPrenom(PRENOM);
    return nomDest;
  }
