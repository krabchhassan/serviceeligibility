Feature: BLUE-6764 Génération des droits avec résiliation + changement BOBB + PW qui ne ferme pas les droits de l'ancien produit à tort + annulDroitsOffline
  ...
  Background:
    ...
    # R - Fermeture à fin %%LAST_YEAR%% avant de réouvrir en %%CURRENT_YEAR%%
    ...
    Given I create a contract element from a file "gt_6764_changementProduit"

  @smokeTests @6764 @caseConsolidation
  Scenario: BLUE-6764 - Test changement de BOBB + PW : Renouvellement %%LAST2_YEARS%% + %%LAST_YEAR%% + %%CURRENT_YEAR%% puis changement de produit + envoi event contrat + annulDroitsOffline doit fermer les droits de l'ancien produit
    ...
    # V - Ouverture du nouveau produitc
    Then there is 4 rightsDomains and the different rightsDomains has this values with indice 9
      | garantie   | domaine | codeProduit       | debut                  | fin                    | finOnline | periodeFermetureDebut | periodeFermetureFin |
      | GEAS20998B | HOSP    | 1472_AJOUT_01_P02 | %%CURRENT_YEAR%%-01-01 | %%CURRENT_YEAR%%-12-31 | null      | null                  | null                |
      | GEAS20998B | AUDI    | 1472_AJOUT_01_P02 | %%CURRENT_YEAR%%-01-01 | %%CURRENT_YEAR%%-12-31 | null      | null                  | null                |
      | GEAS20998B | DENT    | 1472_AJOUT_01_P02 | %%CURRENT_YEAR%%-01-01 | %%CURRENT_YEAR%%-12-31 | null      | null                  | null                |
      | GEAS20998B | PHAR    | 1472_AJOUT_01_P02 | %%CURRENT_YEAR%%-01-01 | %%CURRENT_YEAR%%-12-31 | null      | null                  | null                |
    ...
    Then there is 4 rightsDomains and the different rightsDomains has this values with indice 11
      | garantie   | domaine | codeProduit       | debut                  | fin                 | finOnline           | periodeFermetureDebut  | periodeFermetureFin |
      | GEAS20998B | HOSP    | 1472_AJOUT_01_P02 | %%CURRENT_YEAR%%-01-01 | %%LAST_YEAR%%-12-31 | %%LAST_YEAR%%-12-31 | %%CURRENT_YEAR%%-01-01 | %%LAST_YEAR%%-12-31 |
      | GEAS20998B | AUDI    | 1472_AJOUT_01_P02 | %%CURRENT_YEAR%%-01-01 | %%LAST_YEAR%%-12-31 | %%LAST_YEAR%%-12-31 | %%CURRENT_YEAR%%-01-01 | %%LAST_YEAR%%-12-31 |
      | GEAS20998B | DENT    | 1472_AJOUT_01_P02 | %%CURRENT_YEAR%%-01-01 | %%LAST_YEAR%%-12-31 | %%LAST_YEAR%%-12-31 | %%CURRENT_YEAR%%-01-01 | %%LAST_YEAR%%-12-31 |
      | GEAS20998B | PHAR    | 1472_AJOUT_01_P02 | %%CURRENT_YEAR%%-01-01 | %%LAST_YEAR%%-12-31 | %%LAST_YEAR%%-12-31 | %%CURRENT_YEAR%%-01-01 | %%LAST_YEAR%%-12-31 |
    ...
    Then the expected contract TP is identical to "contrattp-6764" content


  @smokeTests @6764 @caseConsolidation
  Scenario: BLUE-6764 - Test changement de BOBB + PW : Renouvellement %%LAST2_YEARS%% + %%LAST_YEAR%% + %%CURRENT_YEAR%% puis changement de produit + annulDroitsOffline doit fermer les droits de l'ancien produit
    And I create TP card parameters from file "parametrageTP6764_annulDroitOffline"
    Then I renew the rights on "%%CURRENT_YEAR%%-02-18" with mode "RDO"
