Feature: Get Pau V4 for carences CAS2

  @smokeTests @pauv5 @carences @caseConsolidation
  Scenario: Cas 2 : Adhésion à une garantie avec une carence et un droit de remplacement
    ...
    When I send a test contract from file "contractForPauV5/createServicePrestationCas2"
    Then I wait for 1 declaration
    Then there is 8 rightsDomains and the different rightsDomains has this values
      | garantie | domaine | debut                  | fin                    | finOnline              |
      | GT_BASE2 | PHAR    | %%CURRENT_YEAR%%-01-01 | %%CURRENT_YEAR%%-12-31 | null                   |
      | GT_REMP2 | HOSP    | %%CURRENT_YEAR%%-01-01 | %%CURRENT_YEAR%%-03-31 | %%CURRENT_YEAR%%-03-31 |
      | GT_REMP2 | DENT    | %%CURRENT_YEAR%%-01-01 | %%CURRENT_YEAR%%-03-31 | %%CURRENT_YEAR%%-03-31 |
      | GT_REMP2 | OPTI    | %%CURRENT_YEAR%%-01-01 | %%CURRENT_YEAR%%-03-31 | %%CURRENT_YEAR%%-03-31 |
      | GT_BASE2 | HOSP    | %%CURRENT_YEAR%%-04-01 | %%CURRENT_YEAR%%-12-31 | null                   |
      | GT_BASE2 | DENT    | %%CURRENT_YEAR%%-04-01 | %%CURRENT_YEAR%%-12-31 | null                   |
      | GT_BASE2 | OPTI    | %%CURRENT_YEAR%%-04-01 | %%CURRENT_YEAR%%-12-31 | null                   |
      | GT_BASE2 | APDE    | %%CURRENT_YEAR%%-04-01 | %%CURRENT_YEAR%%-12-31 | null                   |
    ...
    Then the expected contract TP is identical to "carences/contrattpcarence2" content
#    Then there is 5 domaineDroits on warranty 0 and the different domaineDroits has this values
